 ...
 * 
 */
public class RequestClassifier implements RequestView {
	...
	private static final String[] COLUMNS_REQUEST = new String[] { " ", // one
																		// space
																		// for
																		// blank
																		// header
	...
	public RequestClassifier() {
		requestModel = new ObjectTableModel(COLUMNS_REQUEST, RowResult.class, // The
																				// object
																				// used
																				// for
																				// each
				...
				new Functor[] { null, null }, new Class[] { String.class,
						String.class }, false);
		paramsModel = new ObjectTableModel(COLUMNS_PARAMS, RowResult.class, // The
																			// object
																			// used
																			// for
																			// each
				...
				new Functor[] { null, null }, new Class[] { String.class,
						String.class }, false);
		headersModel = new ObjectTableModel(COLUMNS_HEADERS, RowResult.class, // The
																				// object
																				// used
																				// for
																				// each
	 ...
	 * 
	 * @see
	 * org.apache.jmeter.visualizers.request.RequestView#setSamplerResult(java
	 * .lang.Object)
	 */
	...
	public void setSamplerResult(Object objectResult) {
		...
		if (objectResult instanceof HTTPSampleResult) {
			...
			System.out.println("Recebi o resultado "
					+ ((HTTPSampleResult) objectResult).getSampleLabel());
			...
			requestModel.addRow(new RowResult(JMeterUtils
					.getResString("view_results_table_request_http_method"), //$NON-NLS-1$
			...
			if (hUrl != null) { // can be null - e.g. if URL was invalid
				requestModel
						.addRow(new RowResult(
								...
								hUrl.getProtocol()));
				requestModel.addRow(new RowResult(JMeterUtils
						.getResString("view_results_table_request_http_host"), //$NON-NLS-1$
						hUrl.getHost()));
				int port = hUrl.getPort() == -1 ? hUrl.getDefaultPort() : hUrl
						.getPort();
				requestModel.addRow(new RowResult(JMeterUtils
						.getResString("view_results_table_request_http_port"), //$NON-NLS-1$
						Integer.valueOf(port)));
				requestModel.addRow(new RowResult(JMeterUtils
						.getResString("view_results_table_request_http_path"), //$NON-NLS-1$
						hUrl.getPath()));
				...
				queryGet = RequestClassifier.decodeQuery(queryGet);
				if (queryGet != null) {
					Set<Entry<String, String>> keys = RequestClassifier
							.getQueryMap(queryGet).entrySet();
					for (Entry<String, String> entry : keys) {
						paramsModel.addRow(new RowResult(entry.getKey(), entry
								.getValue()));
					}
				}
			}
			...
			if (cookie != null && cookie.length() > 0) {
				headersModel
						.addRow(new RowResult(
								JMeterUtils
										.getParsedLabel("view_results_table_request_http_cookie"), //$NON-NLS-1$
								sampleResult.getCookies()));
			}
			// Parsed request headers
			LinkedHashMap<String, String> lhm = JMeterUtils
					.parseHeaders(sampleResult.getRequestHeaders());
			for (Iterator<Map.Entry<String, String>> iterator = lhm.entrySet()
					.iterator(); iterator.hasNext();) {
				Map.Entry<String, String> entry = iterator.next();
				headersModel.addRow(new RowResult(entry.getKey(), entry
						.getValue()));
			}
			...
			requestModel
					.addRow(new RowResult("", //$NON-NLS-1$
							JMeterUtils
									.getResString("view_results_table_request_http_nohttp"))); //$NON-NLS-1$
		}
	}
	...
	public static Map<String, String> getQueryMap(String query) {
		...
		for (String param : params) {
			...
			if (paramSplit.length > 1) {
				// We use substring to keep = sign (Bug 54055), we are sure = is
				// present
	...
	private Component createRequestPane() {
		...
		tableRequest.setToolTipText(JMeterUtils
				.getResString("textbox_tooltip_cell")); // $NON-NLS-1$
		...
		tableParams.setToolTipText(JMeterUtils
				.getResString("textbox_tooltip_cell")); // $NON-NLS-1$
		...
		tableHeaders.setToolTipText(JMeterUtils
				.getResString("textbox_tooltip_cell")); // $NON-NLS-1$
		...
		paneParsed.setResizeWeight(0.25); // set split ratio (only 5 lines to
											// display)
