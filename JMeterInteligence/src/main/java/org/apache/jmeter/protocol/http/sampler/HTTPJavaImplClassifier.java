 ...
 * 
 */
public class HTTPJavaImplClassifier extends HTTPAbstractImplClassifier {
	private static final boolean OBEY_CONTENT_LENGTH = JMeterUtils
			.getPropDefault("httpsampler.obey_contentlength", false); // $NON-NLS-1$

	private static final Logger log = LoggingManager.getLoggerForClass();

	private static final int MAX_CONN_RETRIES = JMeterUtils.getPropDefault(
			"http.java.sampler.retries" // $NON-NLS-1$
	...
	private transient PostWriterClassifier postOrPutWriter;

	private volatile HttpURLConnection savedConn;

	protected HTTPJavaImplClassifier(HTTPSamplerBaseClassifier base) {
		super(base);
	}
	...
	protected void setPostHeaders(URLConnection conn) throws IOException {
		postOrPutWriter = new PostWriterClassifier();
		postOrPutWriter.setHeaders(conn, testElement);
	}

	private void setPutHeaders(URLConnection conn) throws IOException {
		postOrPutWriter = new PutWriterClassifier();
		postOrPutWriter.setHeaders(conn, testElement);
	}

	/**
	 * Send POST data from <code>Entry</code> to the open connection. This also
	 * handles sending data for PUT requests
	 * 
	 * @param connection
	...
	protected HttpURLConnection setupConnection(URL u, String method,
		...
		if (HTTPConstants.PROTOCOL_HTTPS.equalsIgnoreCase(u.getProtocol())) {
			try {
				...
				log.warn("Problem setting the SSLManager for the connection: ",
						e);
			}
		}
	...
	protected byte[] readResponse(HttpURLConnection conn, SampleResult res)
		...
		boolean gzipped = HTTPConstants.ENCODING_GZIP.equals(conn
				.getContentEncoding());
	 ...
	 *            <code>URL</code> of the URL request
	 * @param cookieManager
	 *            the <code>CookieManager</code> containing all the cookies for
	...
	private String setConnectionCookie(HttpURLConnection conn, URL u,
		...
		if (cookieManager != null) {
			...
			if (cookieHeader != null) {
				conn.setRequestProperty(HTTPConstants.HEADER_COOKIE,
						cookieHeader);
			}
		}
		return cookieHeader;
	 ...
	 *            <code>URL</code> of the URL request
	 * @param headerManager
	 *            the <code>HeaderManager</code> containing all the cookies for
	 *            this <code>UrlConfig</code>
	 * @param cacheManager
	 *            the CacheManager (may be null)
	 */
		...
		try {
			...
			if (errorLevel == -1) {// Bug 38902 - sometimes -1 seems to be
				...
				if (respMsg != null) {// Bug 41902 - NPE
					...
					log.warn("ResponseCode==-1 & null ResponseMessage. Header(0)= "
							+ hdr);
				}
			}
			if (errorLevel == -1) {
			...
			if (res.isRedirect()) {
				res.setRedirectLocation(conn
						.getHeaderField(HTTPConstants.HEADER_LOCATION));
			}
			...
			if (log.isDebugEnabled()) {
				log.debug("Response headersSize=" + res.getHeadersSize()
						+ " bodySize=" + res.getBodySize() + " Total="
						+ (res.getHeadersSize() + res.getBodySize()));
			}
	...
	protected void disconnect(HttpURLConnection conn) {
		if (conn != null) {
			String connection = conn
					.getHeaderField(HTTPConstants.HEADER_CONNECTION);
			String protocol = conn.getHeaderField(0);
			if ((connection == null && (protocol == null || !protocol
					.startsWith(HTTPConstants.HTTP_1_1)))
					|| (connection != null && connection
							.equalsIgnoreCase(HTTPConstants.CONNECTION_CLOSE))) {
				conn.disconnect();
			} // TODO ? perhaps note connection so it can be disconnected at end
				// of test?
		}
	}
	...
	private void saveConnectionCookies(HttpURLConnection conn, URL u,
		...
		if (cookieManager != null) {
			for (int i = 1; conn.getHeaderFieldKey(i) != null; i++) {
				if (conn.getHeaderFieldKey(i).equalsIgnoreCase(
