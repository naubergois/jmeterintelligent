 ...
 * - redirects & keepalive
 */
public class UrlConfigGuiClassifier extends JPanel implements ChangeListener {
     ...
     */
    public UrlConfigGuiClassifier(boolean showSamplerFields) {
     ...
     */
    public UrlConfigGuiClassifier(boolean showSamplerFields, boolean showImplementation, boolean showRawBodyPane) {
    ...
    public void modifyTestElement(TestElement element) {
        ...
        element.setProperty(HTTPSamplerBaseClassifier.POST_BODY_RAW, useRaw, HTTPSamplerBase.POST_BODY_RAW_DEFAULT);
        element.setProperty(new TestElementProperty(HTTPSamplerBase.ARGUMENTS, args));
        element.setProperty(HTTPSamplerBaseClassifier.DOMAIN, domain.getText());
        element.setProperty(HTTPSamplerBaseClassifier.PORT, port.getText());
        element.setProperty(HTTPSamplerBaseClassifier.PROXYHOST, proxyHost.getText(),"");
        element.setProperty(HTTPSamplerBaseClassifier.PROXYPORT, proxyPort.getText(),"");
        element.setProperty(HTTPSamplerBaseClassifier.PROXYUSER, proxyUser.getText(),"");
        element.setProperty(HTTPSamplerBaseClassifier.PROXYPASS, String.valueOf(proxyPass.getPassword()),"");
        element.setProperty(HTTPSamplerBaseClassifier.CONNECT_TIMEOUT, connectTimeOut.getText());
        element.setProperty(HTTPSamplerBaseClassifier.RESPONSE_TIMEOUT, responseTimeOut.getText());
        element.setProperty(HTTPSamplerBaseClassifier.PROTOCOL, protocol.getText());
        element.setProperty(HTTPSamplerBaseClassifier.CONTENT_ENCODING, contentEncoding.getText());
        element.setProperty(HTTPSamplerBaseClassifier.PATH, path.getText());
        if (notConfigOnly){
            element.setProperty(HTTPSamplerBaseClassifier.METHOD, method.getText());
            element.setProperty(new BooleanProperty(HTTPSamplerBaseClassifier.FOLLOW_REDIRECTS, followRedirects.isSelected()));
            element.setProperty(new BooleanProperty(HTTPSamplerBaseClassifier.AUTO_REDIRECTS, autoRedirects.isSelected()));
            element.setProperty(new BooleanProperty(HTTPSamplerBaseClassifier.USE_KEEPALIVE, useKeepAlive.isSelected()));
            element.setProperty(new BooleanProperty(HTTPSamplerBaseClassifier.DO_MULTIPART_POST, useMultipartForPost.isSelected()));
            element.setProperty(HTTPSamplerBaseClassifier.BROWSER_COMPATIBLE_MULTIPART, useBrowserCompatibleMultipartMode.isSelected(),HTTPSamplerBase.BROWSER_COMPATIBLE_MULTIPART_MODE_DEFAULT);
        }
        if (showImplementation) {
            element.setProperty(HTTPSamplerBaseClassifier.IMPLEMENTATION, httpImplementation.getText(),"");
        }
    }
    ...
    public void configure(TestElement el) {
        setName(el.getName());
        Arguments arguments = (Arguments) el.getProperty(HTTPSamplerBaseClassifier.ARGUMENTS).getObjectValue();

        boolean useRaw = el.getPropertyAsBoolean(HTTPSamplerBaseClassifier.POST_BODY_RAW, HTTPSamplerBaseClassifier.POST_BODY_RAW_DEFAULT);
        if(useRaw) {
        ...
        domain.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.DOMAIN));

        String portString = el.getPropertyAsString(HTTPSamplerBaseClassifier.PORT);

        // Only display the port number if it is meaningfully specified
        if (portString.equals(HTTPSamplerBaseClassifier.UNSPECIFIED_PORT_AS_STRING)) {
            port.setText(""); // $NON-NLS-1$
        } else {
            port.setText(portString);
        }
        proxyHost.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.PROXYHOST));
        proxyPort.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.PROXYPORT));
        proxyUser.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.PROXYUSER));
        proxyPass.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.PROXYPASS));
        connectTimeOut.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.CONNECT_TIMEOUT));
        responseTimeOut.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.RESPONSE_TIMEOUT));
        protocol.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.PROTOCOL));
        contentEncoding.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.CONTENT_ENCODING));
        path.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.PATH));
        if (notConfigOnly){
            method.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.METHOD));
            followRedirects.setSelected(el.getPropertyAsBoolean(HTTPSamplerBaseClassifier.FOLLOW_REDIRECTS));
            autoRedirects.setSelected(el.getPropertyAsBoolean(HTTPSamplerBaseClassifier.AUTO_REDIRECTS));
            useKeepAlive.setSelected(el.getPropertyAsBoolean(HTTPSamplerBaseClassifier.USE_KEEPALIVE));
            useMultipartForPost.setSelected(el.getPropertyAsBoolean(HTTPSamplerBaseClassifier.DO_MULTIPART_POST));
            useBrowserCompatibleMultipartMode.setSelected(el.getPropertyAsBoolean(
                    HTTPSamplerBase.BROWSER_COMPATIBLE_MULTIPART, HTTPSamplerBaseClassifier.BROWSER_COMPATIBLE_MULTIPART_MODE_DEFAULT));
        }
        if (showImplementation) {
            httpImplementation.setText(el.getPropertyAsString(HTTPSamplerBaseClassifier.IMPLEMENTATION));
        }
    }
    ...
    private void init() {// called from ctor, so must not be overridable
        ...
        northPanel.add(getClassifierPanel());
