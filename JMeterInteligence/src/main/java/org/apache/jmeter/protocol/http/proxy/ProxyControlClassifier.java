...
import org.apache.jmeter.protocol.http.sampler.DaemonClassifier;
 ...
 */
public class ProxyControlClassifier extends GenericController {
	...
	private static final String TRANSACTION_CONTROLLER_GUI = TransactionControllerGui.class
			.getName();

	private static final String LOGIC_CONTROLLER_GUI = LogicControllerGui.class
			.getName();

	private static final String HEADER_PANEL = HeaderPanel.class.getName();

	private transient DaemonClassifier server;
	...
	private static final long sampleGap = JMeterUtils.getPropDefault(
			"proxy.pause", 1000); // $NON-NLS-1$
	...
	private AtomicBoolean samplerRedirectAutomatically = new AtomicBoolean(
			false);
	...
	private AtomicBoolean regexMatch = new AtomicBoolean(false);// Should we
																// match using
																// regexes?
	...
	public void setIncludeList(Collection<String> list) {
		setProperty(new CollectionProperty(INCLUDE_LIST, new HashSet<String>(
				list)));
	}

	public void setExcludeList(Collection<String> list) {
		setProperty(new CollectionProperty(EXCLUDE_LIST, new HashSet<String>(
				list)));
	}
	...
	public String getSamplerTypeName() {
		// Convert the old numeric types - just in case someone wants to reload
		// the workbench
		String type = getPropertyAsString(SAMPLER_TYPE_NAME);
		if (SAMPLER_TYPE_HTTP_SAMPLER_JAVA.equals(type)) {
			type = HTTPSamplerFactoryClassifier.IMPL_JAVA;
		} else if (SAMPLER_TYPE_HTTP_SAMPLER_HC3_1.equals(type)) {
			type = HTTPSamplerFactoryClassifier.IMPL_HTTP_CLIENT3_1;
		} else if (SAMPLER_TYPE_HTTP_SAMPLER_HC4.equals(type)) {
			type = HTTPSamplerFactoryClassifier.IMPL_HTTP_CLIENT4;
		}
		return type;
	}
	...
	public void startProxy() throws IOException {
		...
		try {
			if (learning) {
				server = new DaemonClassifier(getPort(), this,
						ProxyClassifier.class, this.training);
			} else {
				server = new DaemonClassifier(getPort(), this);
			}
			server.start();
	 ...
	 */
	public synchronized void deliverSampler(
			final HTTPSamplerBaseClassifier sampler,
	...
	boolean filterUrl(HTTPSamplerBaseClassifier sampler) {
	...
	// Package protected to allow test case access
	/**
	 * Filter the response based on the content type. If no include nor exclude
	 * filter is specified, the result will be included
	 * 
	 * @param result
	 *            the sample result to check, true means result will be kept
	 */
	 ...
	 */
	private final boolean testPattern(String expression,
			String sampleContentType, boolean expectedToMatch) {
		if (expression != null && expression.length() > 0) {
			if (log.isDebugEnabled()) {
				log.debug("Testing Expression : " + expression
						+ " on sampleContentType:" + sampleContentType
	...
	/**
	 * Helper method to add a Divider Called from Application Thread that needs
	 * to update GUI (JMeterTreeModel)
	 */
	private void addDivider(final JMeterTreeModel model,
	...
	/**
	 * Helper method to add a Simple Controller to contain the samplers. Called
	 * from Application Thread that needs to update GUI (JMeterTreeModel)
	 * 
	 * @param model
	 ...
	 */
	private void addSimpleController(final JMeterTreeModel model,
			final JMeterTreeNode node, String name)
	 ...
	 * @throws InterruptedException
	 */
	private void addTransactionController(final JMeterTreeModel model,
			final JMeterTreeNode node, String name)
			throws InterruptedException, InvocationTargetException {
	 ...
	 * in the timer's configuration with the provided deltaT. Called from AWT
	 * Event thread
	 * 
	 * @param model
	 *            Test component tree model
	 ...
	 */
	private void addTimers(JMeterTreeModel model, JMeterTreeNode node,
			long deltaT) {
	...
	private void placeSampler(final HTTPSamplerBaseClassifier sampler,
			final TestElement[] subConfigs, JMeterTreeNode myTarget) {
		try {
			final JMeterTreeModel treeModel = GuiPackage.getInstance()
					.getTreeModel();
			...
			if (deltaT > sampleGap) {
				if (!myTarget.isLeaf()
						&& cachedGroupingMode == GROUPING_ADD_SEPARATORS) {
			...
			JMeterUtils.runSafe(new Runnable() {
				...
				public void run() {
					try {
						final JMeterTreeNode newNode = treeModel.addComponent(
								sampler, myTargetFinal);
						...
						for (int i = 0; subConfigs != null
								&& i < subConfigs.length; i++) {
							if (subConfigs[i] instanceof HeaderManager) {
								final TestElement headerManager = subConfigs[i];
								headerManager.setProperty(
										TestElement.GUI_CLASS, HEADER_PANEL);
	 ...
	 */
	private void removeValuesFromSampler(HTTPSamplerBaseClassifier sampler,
			Collection<ConfigTestElement> configurations) {
		for (PropertyIterator props = sampler.propertyIterator(); props
				.hasNext();) {
			...
			for (Iterator<ConfigTestElement> configs = configurations
					.iterator(); configs.hasNext();) {
	...
	private String generateMatchUrl(HTTPSamplerBaseClassifier sampler) {
	...
	private void replaceValues(TestElement sampler, TestElement[] configs,
		...
		for (Iterator<Arguments> vars = variables.iterator(); vars.hasNext();) {
			...
			for (Iterator<String> vals = map.values().iterator(); vals
					.hasNext();) {
