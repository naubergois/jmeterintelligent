...
public class ProxyControlGuiClassifier extends LogicControllerGui implements
		JMeterGUIComponent, ActionListener, ItemListener, KeyListener,
		UnsharedComponent {
	...
	private static final String SUGGESTED_EXCLUSIONS = JMeterUtils
			.getPropDefault("proxy.excludes.suggested",
					".*\\.js;.*\\.css;.*\\.swf;.*\\.gif;.*\\.png;.*\\.jpg;.*\\.bmp"); // $NON-NLS-1$

	private JTextField portField;

	private JCheckBox learning;

	private JCheckBox trainning;

	public JCheckBox getTrainning() {
		return trainning;
	}

	public void setTrainning(JCheckBox trainning) {
	...
	private ProxyControlClassifier model;
	...
	public ProxyControlGuiClassifier() {
		super();
	...
	protected ProxyControlClassifier makeProxyControl() {
		...
		ProxyControlClassifier local = new ProxyControlClassifier();
		return local;
	}
	...
	public void modifyTestElement(TestElement el) {
		...
		if (el instanceof ProxyControlClassifier) {
			model = (ProxyControlClassifier) el;
			...
			if (samplerTypeName.getSelectedIndex() < HTTPSamplerFactory
					.getImplementations().length) {
				model.setSamplerTypeName(HTTPSamplerFactory
						.getImplementations()[samplerTypeName
						.getSelectedIndex()]);
			} else {
				model.setSamplerTypeName(USE_DEFAULT_HTTP_IMPL);
			}
			model.setSamplerRedirectAutomatically(samplerRedirectAutomatically
					.isSelected());
			...
			TreeNodeWrapper nw = (TreeNodeWrapper) targetNodes
					.getSelectedItem();
			if (nw == null) {
	...
	protected void setIncludeListInProxyControl(ProxyControlClassifier element) {
		List<String> includeList = getDataList(includeModel, INCLUDE_COL);
		element.setIncludeList(includeList);
	}

	protected void setExcludeListInProxyControl(ProxyControlClassifier element) {
	...
	public void configure(TestElement element) {
		...
		model = (ProxyControlClassifier) element;
		...
		samplerRedirectAutomatically.setSelected(model
	...
	protected void addFromClipboard(JTable table) {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Could not add retrieve "
					+ DataFlavor.stringFlavor.getHumanPresentableName()
					+ " from clipboard" + ufe.getLocalizedMessage(), "Error",
					JOptionPane.ERROR_MESSAGE);
		}
	}
		...
		if (fieldName.equals(PORTFIELD)) {
			try {
				...
				if (length > 0) {
					...
					portField.setText(portField.getText().substring(0,
							length - 1));
				}
			}
			enableRestart();
	...
	private JPanel createTestPlanContentPanel() {
		...
		addAssertions = new JCheckBox(
				JMeterUtils.getResString("proxy_assertions")); // $NON-NLS-1$
	...
	private JPanel createHTTPSamplerPanel() {
		...
		JLabel label2 = new JLabel(
				JMeterUtils.getResString("proxy_sampler_type")); // $NON-NLS-1$
		label2.setLabelFor(samplerTypeName);

		samplerRedirectAutomatically = new JCheckBox(
				JMeterUtils.getResString("follow_redirects_auto")); // $NON-NLS-1$
		...
		samplerFollowRedirects = new JCheckBox(
				JMeterUtils.getResString("follow_redirects")); // $NON-NLS-1$
		...
		samplerDownloadImages = new JCheckBox(
		...
		learning = new JCheckBox();
		panel.add(learning);
		panel.add(new JLabel("learning"));

		trainning = new JCheckBox();
		panel.add(trainning);
		panel.add(new JLabel("trainning"));
	...
	private JPanel createGroupingPanel() {
		...
		m.addElement(JMeterUtils
				.getResString("grouping_in_transaction_controllers")); // $NON-NLS-1$
	...
	private JPanel createContentTypePanel() {
		...
		JLabel labelInclude = new JLabel(
				JMeterUtils.getResString("proxy_content_type_include")); // $NON-NLS-1$
		labelInclude.setLabelFor(contentTypeInclude);
		// Default value
		contentTypeInclude.setText(JMeterUtils
				.getProperty("proxy.content_type_include")); // $NON-NLS-1$

		contentTypeExclude = new JTextField(35);
		contentTypeExclude.addKeyListener(this);
		contentTypeExclude.setName(ENABLE_RESTART);
		JLabel labelExclude = new JLabel(
				JMeterUtils.getResString("proxy_content_type_exclude")); // $NON-NLS-1$
		labelExclude.setLabelFor(contentTypeExclude);
		// Default value
		contentTypeExclude.setText(JMeterUtils
				.getProperty("proxy.content_type_exclude")); // $NON-NLS-1$
	...
	private JPanel createIncludePanel() {
		...
		includeTable.getTableHeader().setDefaultRenderer(
		...
		panel.add(
				createTableButtonPanel(ADD_INCLUDE, DELETE_INCLUDE,
						ADD_TO_INCLUDE_FROM_CLIPBOARD, null),
				BorderLayout.SOUTH);
	...
	private JPanel createExcludePanel() {
		...
		excludeTable.getTableHeader().setDefaultRenderer(
				new HeaderAsPropertyRenderer());
		...
		panel.add(
				createTableButtonPanel(ADD_EXCLUDE, DELETE_EXCLUDE,
						ADD_TO_EXCLUDE_FROM_CLIPBOARD, ADD_SUGGESTED_EXCLUDES),
				BorderLayout.SOUTH);

		return panel;
	}

	private JPanel createTableButtonPanel(String addCommand,
			String deleteCommand, String copyFromClipboard,
		 ...
		 * clipboard.
		 */
		JButton addFromClipboard = new JButton(
				JMeterUtils.getResString("add_from_clipboard")); // $NON-NLS-1$
		...
		if (addSuggestedExcludes != null) {
			...
			JButton addFromSuggestedExcludes = new JButton(
					JMeterUtils.getResString("add_from_suggested_excludes")); // $NON-NLS-1$
	...
	private void reinitializeTargetCombo() {
		...
		if (gp != null) {
			root = (JMeterTreeNode) GuiPackage.getInstance().getTreeModel()
					.getRoot();
			targetNodesModel.addElement(new TreeNodeWrapper(null, JMeterUtils
					.getResString("use_recording_controller"))); // $NON-NLS-1$
		...
		for (int i = 0; i < targetNodesModel.getSize(); i++) {
			...
			log.debug("Selecting item " + choice + " for model " + model
					+ " in " + this);
